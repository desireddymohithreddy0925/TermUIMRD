...
import { type App, type Style, type Rect, mergeStyles, defaultStyle, styleToCellAttrs } from '@termuijs/core';
...
export class TooltipOverlay extends Widget {
    ...
    override updateRect(rect: Rect): void {
        ...
        // The tooltip's rect is fixed to cursor/widget position, but we update it if needed
        if (this._textWidget) {
            this._textWidget.updateRect({
                x: this._tooltipX,
                y: this._tooltipY,
                width: this._textWidget.rect.width,
                height: this._textWidget.rect.height
            });
        }
    }
